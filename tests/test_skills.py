...
def test_no_skills_dir_returns_empty(jail):
    assert discover_skills(jail) == {}


def test_parses_project_skill(jail):
    _write_skill(
        jail / ".clawd/skills",
        "tdd.md",
...
def test_skipped_when_frontmatter_missing(jail):
...
def test_skipped_when_name_missing(jail):
    _write_skill(
        jail / ".clawd/skills",
        "broken.md",
...
def test_skipped_when_yaml_invalid(jail):
...
def test_user_skills_picked_up(jail, tmp_path, monkeypatch):

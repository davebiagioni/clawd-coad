...
function renderAssistant(node, text) {
  // Sanitize untrusted markdown to a DocumentFragment of DOM nodes — avoids
  // touching innerHTML at all. KaTeX renders math after the DOM is in place.
  const dirty = marked.parse(text);
  const fragment = window.DOMPurify
    ? DOMPurify.sanitize(dirty, {
        RETURN_DOM_FRAGMENT: true,
        USE_PROFILES: { html: true, mathMl: true, svg: true },
      })
    : document.createTextNode(dirty);
